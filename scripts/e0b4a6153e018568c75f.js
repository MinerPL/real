(this.webpackChunkdiscord_app = this.webpackChunkdiscord_app || []).push([
    ["44592"], {
        31745: function(e, t, s) {
            "use strict";
            s.r(t), s.d(t, {
                CirclePlusIcon: function() {
                    return n
                }
            });
            var l = s("920040");
            s("773670");
            var a = s("912557"),
                r = s("75196");
            let n = e => {
                let {
                    width: t = 24,
                    height: s = 24,
                    color: n = a.default.colors.INTERACTIVE_NORMAL,
                    colorClass: i = "",
                    ...u
                } = e;
                return (0, l.jsx)("svg", {
                    ...(0, r.default)(u),
                    xmlns: "http://www.w3.org/2000/svg",
                    width: t,
                    height: s,
                    fill: "none",
                    viewBox: "0 0 24 24",
                    children: (0, l.jsx)("path", {
                        fill: "string" == typeof n ? n : n.css,
                        fillRule: "evenodd",
                        d: "M12 23c6.075 0 11-4.925 11-11S18.075 1 12 1 1 5.925 1 12s4.925 11 11 11Zm0-17a1 1 0 0 1 1 1v4h4a1 1 0 1 1 0 2h-4v4a1 1 0 1 1-2 0v-4H7a1 1 0 1 1 0-2h4V7a1 1 0 0 1 1-1Z",
                        clipRule: "evenodd",
                        className: i
                    })
                })
            }
        },
        397659: function(e, t, s) {
            "use strict";
            s.r(t), s.d(t, {
                HeadphonesSlashIcon: function() {
                    return n
                }
            });
            var l = s("920040");
            s("773670");
            var a = s("912557"),
                r = s("75196");
            let n = e => {
                let {
                    width: t = 24,
                    height: s = 24,
                    color: n = a.default.colors.INTERACTIVE_NORMAL,
                    colorClass: i = "",
                    ...u
                } = e;
                return (0, l.jsx)("svg", {
                    ...(0, r.default)(u),
                    xmlns: "http://www.w3.org/2000/svg",
                    width: t,
                    height: s,
                    fill: "none",
                    viewBox: "0 0 24 24",
                    children: (0, l.jsx)("path", {
                        fill: "string" == typeof n ? n : n.css,
                        fillRule: "evenodd",
                        d: "M22.707 2.707a1 1 0 0 0-1.414-1.414l-20 20a1 1 0 1 0 1.414 1.414l20-20Zm-5.649.234a.478.478 0 0 0-.111-.769A10.954 10.954 0 0 0 12 1C5.925 1 1 5.925 1 12c0 1.78.423 3.46 1.173 4.947a.478.478 0 0 0 .769.111l3.19-3.19c.256-.256.16-.69-.196-.763A4.997 4.997 0 0 0 4.917 13H3.055a9 9 0 0 1 12.662-9.199c.196.09.429.053.582-.1l.76-.76ZM20.2 8.283a.518.518 0 0 1 .1-.582l.76-.76a.478.478 0 0 1 .769.112A10.954 10.954 0 0 1 23 12c0 4.143-2.29 7.75-5.67 9.624-1.272.707-2.73.23-3.55-.742a3.104 3.104 0 0 1-.17-3.782l1.377-1.967A5 5 0 0 1 19.082 13h1.862a8.967 8.967 0 0 0-.746-4.717Zm-10.09 9.609c.237-.238.636-.176.727.147.273.97.062 2.04-.616 2.843-.82.973-2.278 1.448-3.55.742a.164.164 0 0 1-.035-.259l3.473-3.473Z",
                        clipRule: "evenodd",
                        className: i
                    })
                })
            }
        },
        780624: function(e, t, s) {
            "use strict";
            s.r(t), s.d(t, {
                MicrophoneSlashIcon: function() {
                    return n
                }
            });
            var l = s("920040");
            s("773670");
            var a = s("912557"),
                r = s("75196");
            let n = e => {
                let {
                    width: t = 24,
                    height: s = 24,
                    color: n = a.default.colors.INTERACTIVE_NORMAL,
                    colorClass: i = "",
                    ...u
                } = e;
                return (0, l.jsx)("svg", {
                    ...(0, r.default)(u),
                    xmlns: "http://www.w3.org/2000/svg",
                    width: t,
                    height: s,
                    fill: "none",
                    viewBox: "0 0 24 24",
                    children: (0, l.jsx)("path", {
                        fill: "string" == typeof n ? n : n.css,
                        d: "m2.707 22.707 20-20a1 1 0 0 0-1.414-1.414l-20 20a1 1 0 1 0 1.414 1.414ZM10.803 17.319c-.216.216-.106.581.197.62V20H9a1 1 0 1 0 0 2h6a1 1 0 1 0 0-2h-2v-2.062A8.001 8.001 0 0 0 20 10a1 1 0 0 0-2 0 5.976 5.976 0 0 1-1.38 3.828l-.022.027a5.988 5.988 0 0 1-4.274 2.136.524.524 0 0 0-.344.15l-1.177 1.178ZM15.357 4.521c.15-.15.192-.38.084-.562A4 4 0 0 0 8 6v4c-.001.294.03.58.091.857.076.346.498.432.749.181l6.516-6.517ZM5.062 13.985a.473.473 0 0 0 .747.084l.748-.747a.53.53 0 0 0 .087-.614A5.974 5.974 0 0 1 6 10a1 1 0 0 0-2 0c0 1.45.386 2.812 1.062 3.985Z",
                        className: i
                    })
                })
            }
        },
        776863: function(e, t, s) {
            "use strict";
            s.r(t), s.d(t, {
                useChannelsUserCanStartStageIn: function() {
                    return i
                }
            });
            var l = s("498225"),
                a = s("923510"),
                r = s("923959"),
                n = s("957255");

            function i(e) {
                var t;
                let s = null !== (t = null == e ? void 0 : e.id) && void 0 !== t ? t : null;
                return (0, l.useStateFromStoresArray)([r.default, n.default], () => {
                    let e = r.default.getChannels(s)[r.GUILD_VOCAL_CHANNELS_KEY];
                    return e.reduce((e, t) => {
                        let s = t.channel;
                        return s.isGuildStageVoice() ? (function(e) {
                            let t = arguments.length > 1 && void 0 !== arguments[1] ? arguments[1] : n.default;
                            return !!e.isGuildStageVoice() && t.can(a.MODERATE_STAGE_CHANNEL_PERMISSIONS, e)
                        }(t.channel, n.default) && e.push(s), e) : e
                    }, [])
                }, [s])
            }
        },
        844808: function(e, t, s) {
            "use strict";
            s.r(t), s.d(t, {
                default: function() {
                    return d
                }
            });
            var l = s("920040"),
                a = s("773670"),
                r = s("77078"),
                n = s("822516"),
                i = s("757767"),
                u = s("782340");

            function d(e) {
                let {
                    startDate: t,
                    recurrenceRule: s,
                    onRecurrenceChange: d
                } = e, o = a.useMemo(() => (0, n.recurrenceRuleToOption)(t, s), [s, t]), c = function(e) {
                    let t = e.toDate(),
                        s = t.toLocaleString(u.default.getLocale(), {
                            weekday: "long"
                        });
                    return [{
                        value: i.RecurrenceOptions.NONE,
                        label: u.default.Messages.CREATE_EVENT_RECUR_NONE
                    }, {
                        value: i.RecurrenceOptions.WEEKLY,
                        label: u.default.Messages.CREATE_EVENT_RECUR_WEEKLY.format({
                            weekday: s
                        })
                    }, {
                        value: i.RecurrenceOptions.YEARLY,
                        label: u.default.Messages.CREATE_EVENT_RECUR_YEARLY.format({
                            date: t.toLocaleString(u.default.getLocale(), {
                                month: "short",
                                day: "2-digit"
                            })
                        })
                    }, {
                        value: i.RecurrenceOptions.WEEKDAY_ONLY,
                        label: u.default.Messages.CREATE_EVENT_RECUR_WEEKDAYS
                    }]
                }(t), h = e => e.toString();
                return (0, l.jsx)(r.FormItem, {
                    title: u.default.Messages.CREATE_EVENT_RECUR_LABEL,
                    required: !0,
                    children: (0, l.jsx)(r.Select, {
                        placeholder: "gaming",
                        options: c,
                        select: d,
                        serialize: h,
                        isSelected: e => null != o && h(e) === h(o)
                    })
                })
            }
        },
        953143: function(e, t, s) {
            "use strict";
            s.r(t), s.d(t, {
                default: function() {
                    return A
                }
            });
            var l = s("920040");
            s("773670");
            var a = s("377849"),
                r = s.n(a),
                n = s("77078"),
                i = s("812204"),
                u = s("685665"),
                d = s("381546"),
                o = s("351825"),
                c = s("651072"),
                h = s("822516"),
                E = s("844808"),
                f = s("782340"),
                _ = s("856599");

            function A(e) {
                let {
                    className: t,
                    guildId: s,
                    onScheduleChange: a,
                    onRecurrenceChange: A,
                    onTimeChange: g,
                    timeSelected: v = !0,
                    schedule: x,
                    recurrenceRule: p,
                    showEndDate: N = !1,
                    requireEndDate: C = !1,
                    disableStartDateTime: m = !1
                } = e, {
                    analyticsLocations: T
                } = (0, u.default)(i.default.EVENT_SETTINGS), {
                    enabled: I
                } = c.default.useExperiment({
                    guildId: null != s ? s : "",
                    location: T[0]
                }, {
                    autoTrackExposure: !1
                });
                if (null == x) return null;
                let S = null,
                    R = x.startDate,
                    j = r(),
                    L = r().add(h.MAX_DAYS_AHEAD_AN_EVENT_CAN_START, "days"),
                    O = r().add(h.MAX_DAYS_AHEAD_AN_EVENT_CAN_END, "days"),
                    M = e => {
                        a({
                            ...x,
                            endDate: e
                        })
                    };
                return null != R && c.default.trackExposure({
                    guildId: null != s ? s : "",
                    location: T[0]
                }), N && (S = null != x.endDate || C ? (0, l.jsxs)(l.Fragment, {
                    children: [(0, l.jsxs)("div", {
                        className: _.doubleInput,
                        children: [(0, l.jsx)(n.FormItem, {
                            title: f.default.Messages.CREATE_EVENT_END_DATE_LABEL,
                            required: C,
                            children: (0, l.jsx)(n.DateInput, {
                                value: x.endDate,
                                onSelect: M,
                                minDate: x.startDate,
                                maxDate: O
                            })
                        }), (0, l.jsx)(n.FormItem, {
                            title: f.default.Messages.CREATE_EVENT_END_TIME_LABEL,
                            required: C,
                            children: (0, l.jsx)(n.TimeInput, {
                                value: x.endDate,
                                onChange: M
                            })
                        })]
                    }), C ? null : (0, l.jsx)(n.Button, {
                        onClick: () => {
                            M(void 0)
                        },
                        look: n.Button.Looks.BLANK,
                        size: n.Button.Sizes.MIN,
                        children: (0, l.jsxs)("div", {
                            className: _.link,
                            children: [(0, l.jsx)(d.default, {
                                width: 17,
                                height: 17,
                                className: _.removeIcon
                            }), (0, l.jsx)(n.Text, {
                                variant: "text-sm/normal",
                                children: f.default.Messages.CREATE_EVENT_REMOVE_END_TIME_LABEL
                            })]
                        })
                    })]
                }) : (0, l.jsx)(n.Button, {
                    look: n.Button.Looks.BLANK,
                    size: n.Button.Sizes.MIN,
                    onClick: () => {
                        M(r(x.startDate).add(1, "hour"))
                    },
                    children: (0, l.jsxs)("div", {
                        className: _.link,
                        children: [(0, l.jsx)(o.default, {
                            width: 20,
                            height: 20,
                            className: _.addIcon
                        }), (0, l.jsx)(n.Text, {
                            variant: "text-sm/normal",
                            children: f.default.Messages.CREATE_EVENT_ADD_END_TIME_LABEL
                        })]
                    })
                })), (0, l.jsxs)("div", {
                    className: t,
                    children: [(0, l.jsxs)("div", {
                        className: _.doubleInput,
                        children: [(0, l.jsx)(n.FormItem, {
                            title: f.default.Messages.CREATE_EVENT_START_DATE_LABEL,
                            required: !0,
                            children: (0, l.jsx)(n.DateInput, {
                                value: x.startDate,
                                onSelect: e => {
                                    a({
                                        ...x,
                                        startDate: e
                                    })
                                },
                                minDate: j,
                                maxDate: L,
                                disabled: m
                            })
                        }), (0, l.jsx)(n.FormItem, {
                            title: f.default.Messages.CREATE_EVENT_START_TIME_LABEL,
                            required: !0,
                            children: (0, l.jsx)(n.TimeInput, {
                                value: x.startDate,
                                onChange: e => {
                                    e.isValid() && (null == g || g(!0), a({
                                        ...x,
                                        startDate: e
                                    }))
                                },
                                hideValue: !v,
                                disabled: m
                            })
                        })]
                    }), S, I && null != R && null != A && (0, l.jsx)(E.default, {
                        onRecurrenceChange: A,
                        startDate: R,
                        recurrenceRule: p
                    })]
                })
            }
        },
        597517: function(e, t, s) {
            "use strict";
            s.r(t), s.d(t, {
                hideHotspot: function() {
                    return n
                },
                setHotspotOverride: function() {
                    return i
                },
                clearHotspotOverride: function() {
                    return u
                }
            });
            var l = s("913144"),
                a = s("599110"),
                r = s("49111");

            function n(e) {
                a.default.track(r.AnalyticEvents.HOTSPOT_HIDDEN, {
                    hotspot_location: e
                }), l.default.wait(() => {
                    l.default.dispatch({
                        type: "HOTSPOT_HIDE",
                        location: e
                    })
                })
            }

            function i(e, t) {
                l.default.dispatch({
                    type: "HOTSPOT_OVERRIDE_SET",
                    location: e,
                    enabled: t
                })
            }

            function u(e) {
                l.default.dispatch({
                    type: "HOTSPOT_OVERRIDE_CLEAR",
                    location: e
                })
            }
        },
        269596: function(e, t, s) {
            "use strict";
            s.r(t), s.d(t, {
                default: function() {
                    return o
                }
            });
            var l = s("498225"),
                a = s("913144"),
                r = s("197881"),
                n = s("492397");
            let i = new Set,
                u = {};
            class d extends l.default.PersistedStore {
                initialize(e) {
                    null != e && (Array.isArray(e.hiddenHotspots) && (i = new Set(e.hiddenHotspots)), null != e.hotspotOverrides && (u = e.hotspotOverrides))
                }
                hasHotspot(e) {
                    let t = arguments.length > 1 && void 0 !== arguments[1] && arguments[1],
                        s = !t && u[e];
                    return !(n.CONFERENCE_MODE_ENABLED || r.ProcessArgs.isDisallowPopupsSet()) && (s || !i.has(e))
                }
                hasHiddenHotspot(e) {
                    return i.has(e)
                }
                getHotspotOverride(e) {
                    return u[e]
                }
                getState() {
                    return {
                        hiddenHotspots: i,
                        hotspotOverrides: u
                    }
                }
            }
            d.displayName = "HotspotStore", d.persistKey = "hotspots", d.migrations = [e => ({
                hiddenHotspots: null != e ? e : [],
                hotspotOverrides: {}
            })];
            var o = new d(a.default, {
                OVERLAY_INITIALIZE: function(e) {
                    let {
                        hiddenHotspots: t
                    } = e;
                    i = new Set(t)
                },
                HOTSPOT_HIDE: function(e) {
                    let {
                        location: t
                    } = e;
                    if (i.has(t)) return !1;
                    i.add(t)
                },
                HOTSPOT_OVERRIDE_SET: function(e) {
                    let {
                        location: t,
                        enabled: s
                    } = e;
                    u[t] = s
                },
                HOTSPOT_OVERRIDE_CLEAR: function(e) {
                    let {
                        location: t
                    } = e;
                    if (null == u[t]) return !1;
                    delete u[t]
                }
            })
        },
        29846: function(e, t, s) {
            "use strict";
            s.r(t), s.d(t, {
                BlockedUsersNotice: function() {
                    return g
                },
                BlockedUser: function() {
                    return v
                }
            });
            var l = s("920040");
            s("773670");
            var a = s("498225"),
                r = s("912557"),
                n = s("77078"),
                i = s("272030"),
                u = s("328275"),
                d = s("849467"),
                o = s("258078"),
                c = s("823050"),
                h = s("158998"),
                E = s("325861"),
                f = s("151642"),
                _ = s("782340"),
                A = s("200334");
            let g = e => {
                    let {
                        channelId: t
                    } = e, s = (0, f.useStageBlockedUsersCount)(t);
                    return 0 === s ? null : (0, l.jsxs)("div", {
                        className: A.blockedNotice,
                        children: [(0, l.jsx)(u.default, {
                            className: A.blockedIcon,
                            color: r.default.unsafe_rawColors.RED_400.css
                        }), (0, l.jsx)(n.Text, {
                            variant: "text-xs/normal",
                            color: "header-secondary",
                            children: _.default.Messages.STAGE_BLOCKED_USERS_DESCRIPTION_COUNT.format({
                                number: s
                            })
                        }), (0, l.jsx)(n.Clickable, {
                            className: A.blockedButton,
                            onClick: e => {
                                (0, i.openContextMenu)(e, e => (0, l.jsx)(x, {
                                    ...e,
                                    channelId: t
                                }), {
                                    position: "left",
                                    align: "bottom"
                                })
                            },
                            children: _.default.Messages.VIEW_ALL
                        })]
                    })
                },
                v = e => {
                    let {
                        user: t,
                        showStatus: s,
                        speaker: r,
                        channelId: i
                    } = e, u = (0, a.useStateFromStores)([E.default], () => E.default.isModerator(t.id, i)), f = null;
                    return s && (f = r ? _.default.Messages.STAGE_SPEAKER : u ? _.default.Messages.STAGE_MODERATOR_TOOLTIP : _.default.Messages.STAGE_AUDIENCE), (0, l.jsxs)("div", {
                        className: A.user,
                        children: [(0, l.jsx)(c.default, {
                            src: t.getAvatarURL(null, 32),
                            size: n.AvatarSizes.SIZE_32,
                            muted: !1,
                            deafen: !1,
                            speaking: !1,
                            ringing: !1,
                            renderIcon: r ? () => (0, l.jsx)(d.default, {
                                className: A.icon
                            }) : null
                        }, t.id), (0, l.jsxs)("div", {
                            className: A.userInfo,
                            children: [(0, l.jsxs)("div", {
                                className: A.username,
                                children: [(0, l.jsx)(o.default, {
                                    size: s ? o.default.Sizes.SIZE_16 : o.default.Sizes.SIZE_14,
                                    children: h.default.getName(t)
                                }), (0, l.jsx)(o.default, {
                                    size: s ? o.default.Sizes.SIZE_16 : o.default.Sizes.SIZE_14,
                                    color: o.default.Colors.HEADER_SECONDARY,
                                    children: "#".concat(t.discriminator)
                                })]
                            }), (0, l.jsxs)("div", {
                                className: A.username,
                                children: [(0, l.jsx)(n.Text, {
                                    variant: "text-xs/normal",
                                    color: "text-danger",
                                    children: _.default.Messages.BLOCKED
                                }), (0, l.jsxs)(n.Text, {
                                    variant: "text-xs/normal",
                                    color: "text-muted",
                                    children: [" ", "| ", f]
                                })]
                            })]
                        })]
                    })
                },
                x = e => {
                    let {
                        channelId: t
                    } = e, s = (0, f.useStageBlockedUsers)(t);
                    return (0, l.jsx)(n.Scroller, {
                        className: A.container,
                        children: s.map(e => {
                            let {
                                user: s
                            } = e;
                            return (0, l.jsx)(v, {
                                user: s,
                                channelId: t
                            }, s.id)
                        })
                    })
                }
        },
        328275: function(e, t, s) {
            "use strict";
            s.r(t), s.d(t, {
                default: function() {
                    return r
                }
            });
            var l = s("920040");
            s("773670");
            var a = s("75196");

            function r(e) {
                let {
                    width: t = 32,
                    height: s = 32,
                    color: r = "currentColor",
                    ...n
                } = e;
                return (0, l.jsxs)("svg", {
                    ...(0, a.default)(n),
                    width: t,
                    height: s,
                    viewBox: "0 0 16 16",
                    fill: "none",
                    children: [(0, l.jsx)("clipPath", {
                        children: (0, l.jsx)("path", {
                            d: "m.000183 0h16v16h-16z"
                        })
                    }), (0, l.jsx)("g", {
                        clipPath: "url(#a)",
                        children: (0, l.jsx)("path", {
                            d: "m7.90039 1c-3.9 0-6.999999 3.1-6.999999 7s3.099999 7 6.999999 7c3.90001 0 7.00001-3.1 7.00001-7s-3.2-7-7.00001-7zm0 2.3c.8 0 1.6.3 2.30001.7l-6.40001 6.4c-.3-.7-.6-1.5-.6-2.4 0-2.6 2.1-4.7 4.7-4.7zm0 9.4c-.9 0-1.7-.3-2.4-.7l6.40001-6.4c.4.7.7 1.5.7 2.4-.1 2.6-2.2 4.7-4.70001 4.7z",
                            fill: r
                        })
                    })]
                })
            }
        },
        200639: function(e, t, s) {
            "use strict";
            s.r(t), s.d(t, {
                default: function() {
                    return i
                }
            });
            var l = s("920040");
            s("773670");
            var a = s("469563"),
                r = s("397659"),
                n = s("75196"),
                i = (0, a.replaceIcon)(function(e) {
                    let {
                        width: t = 24,
                        height: s = 24,
                        color: a = "currentColor",
                        foreground: r,
                        ...i
                    } = e;
                    return (0, l.jsxs)("svg", {
                        ...(0, n.default)(i),
                        width: t,
                        height: s,
                        viewBox: "0 0 24 24",
                        children: [(0, l.jsx)("path", {
                            d: "M6.16204 15.0065C6.10859 15.0022 6.05455 15 6 15H4V12C4 7.588 7.589 4 12 4C13.4809 4 14.8691 4.40439 16.0599 5.10859L17.5102 3.65835C15.9292 2.61064 14.0346 2 12 2C6.486 2 2 6.485 2 12V19.1685L6.16204 15.0065Z",
                            fill: a
                        }), (0, l.jsx)("path", {
                            d: "M19.725 9.91686C19.9043 10.5813 20 11.2796 20 12V15H18C16.896 15 16 15.896 16 17V20C16 21.104 16.896 22 18 22H20C21.105 22 22 21.104 22 20V12C22 10.7075 21.7536 9.47149 21.3053 8.33658L19.725 9.91686Z",
                            fill: a
                        }), (0, l.jsx)("path", {
                            d: "M3.20101 23.6243L1.7868 22.2101L21.5858 2.41113L23 3.82535L3.20101 23.6243Z",
                            className: r,
                            fill: a
                        })]
                    })
                }, r.HeadphonesSlashIcon)
        },
        97347: function(e, t, s) {
            "use strict";
            s.r(t), s.d(t, {
                default: function() {
                    return i
                }
            });
            var l = s("920040");
            s("773670");
            var a = s("469563"),
                r = s("780624"),
                n = s("75196"),
                i = (0, a.replaceIcon)(function(e) {
                    let {
                        width: t = 24,
                        height: s = 24,
                        color: a = "currentColor",
                        foreground: r,
                        ...i
                    } = e;
                    return (0, l.jsxs)("svg", {
                        ...(0, n.default)(i),
                        width: t,
                        height: s,
                        viewBox: "0 0 24 24",
                        children: [(0, l.jsx)("path", {
                            d: "M6.7 11H5C5 12.19 5.34 13.3 5.9 14.28L7.13 13.05C6.86 12.43 6.7 11.74 6.7 11Z",
                            fill: a
                        }), (0, l.jsx)("path", {
                            d: "M9.01 11.085C9.015 11.1125 9.02 11.14 9.02 11.17L15 5.18V5C15 3.34 13.66 2 12 2C10.34 2 9 3.34 9 5V11C9 11.03 9.005 11.0575 9.01 11.085Z",
                            fill: a
                        }), (0, l.jsx)("path", {
                            d: "M11.7237 16.0927L10.9632 16.8531L10.2533 17.5688C10.4978 17.633 10.747 17.6839 11 17.72V22H13V17.72C16.28 17.23 19 14.41 19 11H17.3C17.3 14 14.76 16.1 12 16.1C11.9076 16.1 11.8155 16.0975 11.7237 16.0927Z",
                            fill: a
                        }), (0, l.jsx)("path", {
                            d: "M21 4.27L19.73 3L3 19.73L4.27 21L8.46 16.82L9.69 15.58L11.35 13.92L14.99 10.28L21 4.27Z",
                            className: r,
                            fill: a
                        })]
                    })
                }, r.MicrophoneSlashIcon)
        },
        351825: function(e, t, s) {
            "use strict";
            s.r(t), s.d(t, {
                default: function() {
                    return n
                }
            });
            var l = s("920040");
            s("773670");
            var a = s("469563"),
                r = s("31745"),
                n = (0, a.replaceIcon)(function(e) {
                    let {
                        width: t = 16,
                        height: s = 16,
                        color: a = "currentColor",
                        className: r,
                        foreground: n
                    } = e;
                    return (0, l.jsx)("svg", {
                        className: r,
                        width: t,
                        height: s,
                        viewBox: "0 0 24 24",
                        children: (0, l.jsx)("path", {
                            className: n,
                            fill: a,
                            d: "M12 2.00098C6.486 2.00098 2 6.48698 2 12.001C2 17.515 6.486 22.001 12 22.001C17.514 22.001 22 17.515 22 12.001C22 6.48698 17.514 2.00098 12 2.00098ZM17 13.001H13V17.001H11V13.001H7V11.001H11V7.00098H13V11.001H17V13.001Z"
                        })
                    })
                }, r.CirclePlusIcon)
        },
        823050: function(e, t, s) {
            "use strict";
            s.r(t), s.d(t, {
                default: function() {
                    return _
                }
            });
            var l, a, r = s("920040"),
                n = s("773670"),
                i = s("575482"),
                u = s.n(i),
                d = s("77078"),
                o = s("200639"),
                c = s("97347"),
                h = s("587974"),
                E = s("351701");
            (l = a || (a = {}))[l.OUTGOING = 0] = "OUTGOING", l[l.INCOMING = 1] = "INCOMING";
            class f extends n.PureComponent {
                renderStatus() {
                    let {
                        muted: e,
                        deafen: t
                    } = this.props, s = t ? o.default : c.default;
                    return e || t ? (0, r.jsx)(s, {
                        className: E.status
                    }, "status") : null
                }
                renderBorder() {
                    let {
                        speaking: e
                    } = this.props;
                    return (0, r.jsx)("div", {
                        className: u(E.border, {
                            [E.speaking]: e
                        })
                    }, "border")
                }
                renderMask() {
                    let {
                        muted: e,
                        speaking: t,
                        deafen: s
                    } = this.props;
                    return e && t ? (0, r.jsxs)(r.Fragment, {
                        children: [this.renderBorder(), this.renderStatus()]
                    }) : e || s ? (0, r.jsx)(r.Fragment, {
                        children: this.renderStatus()
                    }) : (0, r.jsx)(r.Fragment, {
                        children: this.renderBorder()
                    })
                }
                renderVoiceCallAvatar() {
                    let {
                        src: e,
                        disabled: t,
                        ringing: s,
                        ringingType: l,
                        avatarClassName: n,
                        muted: i,
                        deafen: o,
                        size: c,
                        renderIcon: f
                    } = this.props, _ = (0, d.getAvatarSize)(c), A = (0, r.jsx)("img", {
                        src: e,
                        alt: " ",
                        className: u(E.voiceAvatar, n, {
                            [E.ringingOutgoing]: s && l === a.OUTGOING
                        })
                    });
                    if (t) return A;
                    let g = h.MaskIDs.AVATAR_DEFAULT;
                    return null != f ? g = c === d.AvatarSizes.SIZE_32 ? h.MaskIDs.AVATAR_CALL_ICON_32 : h.MaskIDs.AVATAR_CALL_ICON : (i || o) && (g = h.MaskIDs.AVATAR_VOICE_CALL_80), (0, r.jsxs)("div", {
                        className: E.callAvatarMaskContainer,
                        children: [(0, r.jsxs)(h.default, {
                            className: E.callAvatarMask,
                            width: _,
                            height: _,
                            mask: g,
                            children: [null == f ? void 0 : f(), A]
                        }), this.renderMask()]
                    })
                }
                render() {
                    let {
                        size: e,
                        onClick: t,
                        style: s,
                        onContextMenu: l,
                        className: n,
                        ringingType: i,
                        ringing: o
                    } = this.props, c = u(E.wrapper, {
                        [E.clickable]: null != t,
                        [E.ringingOutgoing]: o && i === a.OUTGOING,
                        [E.ringingIncoming]: o && i === a.INCOMING
                    }, n), h = (0, d.getAvatarSize)(e), f = {
                        height: h,
                        width: h,
                        ...s
                    };
                    return (0, r.jsx)("div", {
                        style: f,
                        onContextMenu: l,
                        className: c,
                        children: this.renderVoiceCallAvatar()
                    })
                }
            }
            f.defaultProps = {
                size: d.AvatarSizes.SIZE_80,
                ringingType: a.OUTGOING,
                speaking: !1,
                soundsharing: !1,
                ringing: !1,
                muted: !1,
                deafen: !1,
                hasVideo: !1,
                mirror: !1,
                disabled: !1,
                paused: !1
            }, f.RingingType = a;
            var _ = f
        }
    }
]);